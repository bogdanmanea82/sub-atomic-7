...
import { ItemModifierTierModel } from "@model/entities/item-modifier-tier";
...
describe("ItemModifierTierModel", () => {
  // ── Validation ──────────────────────────────────────────────────────────
  describe("validate", () => {
    it("accepts valid tier input", () => {
      const result = ItemModifierTierModel.validate(VALID_TIER_INPUT);
    ...
    it("accepts tier_index 0 (first tier)", () => {
      expect(() => ItemModifierTierModel.validate({ ...VALID_TIER_INPUT, tier_index: 0 })).not.toThrow();
    });

    it("rejects negative tier_index", () => {
      try {
        ItemModifierTierModel.validate({ ...VALID_TIER_INPUT, tier_index: -1 });
    ...
    it("rejects level_req below 1", () => {
      try {
        ItemModifierTierModel.validate({ ...VALID_TIER_INPUT, level_req: 0 });
        expect(true).toBe(false);
      } catch (error) {
        const err = error as Error & { errors: Record<string, string> };
    ...
    it("rejects level_req above 99", () => {
      try {
        ItemModifierTierModel.validate({ ...VALID_TIER_INPUT, level_req: 100 });
        expect(true).toBe(false);
      } catch (error) {
        const err = error as Error & { errors: Record<string, string> };
        expect(err.errors["level_req"]).toBeDefined();
    ...
    it("rejects negative spawn_weight", () => {
      try {
        ItemModifierTierModel.validate({ ...VALID_TIER_INPUT, spawn_weight: -1 });
        expect(true).toBe(false);
      } catch (error) {
        const err = error as Error & { errors: Record<string, string> };
    ...
    it("rejects missing required fields", () => {
      try {
        ItemModifierTierModel.validate({});
        expect(true).toBe(false);
      } catch (error) {
        const err = error as Error & { errors: Record<string, string> };
    ...
    it("rejects NaN for decimal fields", () => {
      try {
        ItemModifierTierModel.validate({ ...VALID_TIER_INPUT, min_value: NaN });
        expect(true).toBe(false);
      } catch (error) {
        const err = error as Error & { errors: Record<string, string> };
  ...
  describe("serialize", () => {
    it("serializes decimal values to strings for NUMERIC columns", () => {
      const result = ItemModifierTierModel.serialize(VALID_TIER_INPUT, "create");
      expect(result["min_value"]).toBe("1.5");
  ...
  describe("deserialize", () => {
    it("parses NUMERIC string columns back to numbers", () => {
      ...
      const result = ItemModifierTierModel.deserialize(dbRow);
      expect(result.min_value).toBe(1.5);
      expect(result.max_value).toBe(5);
      expect(result.tier_index).toBe(0);
    });
  ...
  describe("prepareCreate", () => {
    it("builds an INSERT query with correct table", () => {
      ...
      const query = ItemModifierTierModel.prepareCreate(input);
  ...
  describe("prepareSelect", () => {
    it("builds a SELECT query for modifier_id condition", () => {
      const query = ItemModifierTierModel.prepareSelect({ modifier_id: "some-uuid" });
  ...
  describe("prepareDelete", () => {
    it("builds a DELETE query for modifier_id condition", () => {
      const query = ItemModifierTierModel.prepareDelete({ modifier_id: "some-uuid" });
