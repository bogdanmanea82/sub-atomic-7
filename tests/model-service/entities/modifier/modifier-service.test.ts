...
describe("ModifierService", () => {
  ...
  describe("addTier", () => {
    ...
    it("appends a tier with auto-assigned tier_index = currentTiers.length", async () => {
      ...
      const result = await ModifierService.addTier(modifierIdForTierTests, {
        min_value: 6,
        max_value: 15,
        level_req: 10,
        spawn_weight: 50,
    ...
    it("returns not_found for a non-existent parent id", async () => {
      const result = await ModifierService.addTier(crypto.randomUUID(), {
    ...
    it("replaces a tier's values at the given index", async () => {
      ...
      const result = await ModifierService.updateTier(modifierIdForTierTests, 0, {
        min_value: 1,
    ...
    it("updateTier returns not_found for a non-existent parent id", async () => {
      const result = await ModifierService.updateTier(crypto.randomUUID(), 0, {
        min_value: 1, max_value: 5, level_req: 1, spawn_weight: 100,
      });
      expect(result.success).toBe(false);
      if (!result.success) {
    ...
    it("updateTier returns not_found for an out-of-bounds tier index", async () => {
      ...
      const result = await ModifierService.updateTier(modifierIdForTierTests, 999, {
        min_value: 1, max_value: 5, level_req: 1, spawn_weight: 100,
      });
      expect(result.success).toBe(false);
      if (!result.success) {
    ...
    it("removes a middle tier and reindexes remaining tiers gaplessly", async () => {
      ...
      const result = await ModifierService.deleteTier(modifierIdForTierTests, 1);
    ...
    it("deleteTier returns not_found for a non-existent parent id", async () => {
      const result = await ModifierService.deleteTier(crypto.randomUUID(), 0);
      expect(result.success).toBe(false);
      if (!result.success) {
        expect(result.stage).toBe("not_found");
      }
    ...
    it("deleteTier returns not_found for an out-of-bounds tier index", async () => {
      ...
      const result = await ModifierService.deleteTier(modifierIdForTierTests, 999);
      expect(result.success).toBe(false);
      if (!result.success) {
        expect(result.stage).toBe("not_found");
      }
    ...
    it("cleanup: delete the tier-ops modifier", async () => {
      ...
      await ModifierService.delete(modifierIdForTierTests);
