...
// Verifies MODIFIER_CONFIG has the correct non-column keys, table name, and field composition.
// Also verifies the Milestone 2 refactor: old fields removed, new mechanical fields present.
// Pure unit tests — no database.

import { describe, it, expect } from "bun:test";
import { MODIFIER_CONFIG } from "../../../../src/config/entities/modifier";

describe("MODIFIER_CONFIG", () => {
  // ── Identity ───────────────────────────────────────────────────────────────

  it("has entity name 'Modifier'", () => {
    expect(MODIFIER_CONFIG.name).toBe("Modifier");
  });

  it("has tableName 'item_modifier' (snake_case of Modifier)", () => {
    expect(MODIFIER_CONFIG.tableName).toBe("item_modifier");
  });

  it("has correct display names", () => {
    expect(MODIFIER_CONFIG.displayName).toBe("Item Modifier");
    expect(MODIFIER_CONFIG.pluralDisplayName).toBe("Item Modifiers");
  });
  ...
  it("read is public, write operations require admin", () => {
    expect(MODIFIER_CONFIG.permissions.read).toBe("public");
    expect(MODIFIER_CONFIG.permissions.create).toBe("admin");
    expect(MODIFIER_CONFIG.permissions.update).toBe("admin");
    expect(MODIFIER_CONFIG.permissions.delete).toBe("admin");
  });
  ...
  it("does NOT have semantic_cat field (replaced by stat.category)", () => {
    const names = MODIFIER_CONFIG.fields.map((f) => f.name);
  ...
  it("has target_stat_id as a required reference to the Stat entity", () => {
    const f = MODIFIER_CONFIG.fields.find((f) => f.name === "target_stat_id");
  ...
  it("has combination_type as a required enum with flat, increased, more", () => {
    const f = MODIFIER_CONFIG.fields.find((f) => f.name === "combination_type");
    expect(f).toBeDefined();
  ...
  it("has roll_shape as a required enum with scalar, range", () => {
    const f = MODIFIER_CONFIG.fields.find((f) => f.name === "roll_shape");
    expect(f).toBeDefined();
    expect(f!.type).toBe("enum");
    expect(f!.required).toBe(true);
    if (f!.type === "enum") {
  ...
  it("has value_min as a required integer with signed range", () => {
    const f = MODIFIER_CONFIG.fields.find((f) => f.name === "value_min");
    expect(f).toBeDefined();
  ...
  it("has value_max as a required integer with signed range", () => {
    const f = MODIFIER_CONFIG.fields.find((f) => f.name === "value_max");
    expect(f).toBeDefined();
    expect(f!.type).toBe("integer");
    expect(f!.required).toBe(true);
    if (f!.type === "integer") {
  ...
  it("has modifier_group as a required string field", () => {
    const f = MODIFIER_CONFIG.fields.find((f) => f.name === "modifier_group");
    expect(f).toBeDefined();
  ...
  it("has display_template as a required string field using textarea", () => {
    const f = MODIFIER_CONFIG.fields.find((f) => f.name === "display_template");
    expect(f).toBeDefined();
    expect(f!.type).toBe("string");
    expect(f!.required).toBe(true);
