// src/model-service/entities/item-modifie./modifier-service.ts
...
const MACHINE_NAME_ERROR = "A Modifier with this machine_name already exists in this subcategory.";
...
async function checkUniqueness(
  ...
  if (typeof machineName === "string" && machineName.trim() !== "") {
    const mnCheck = await checkFieldUniqueness(db, ModifierModel, "machine_name", machineName, MACHINE_NAME_ERROR, scope, excludeId);
...
const _core = {
  ...
  async findById(id: string): Promise<SelectWorkflowResult<ModifierWithTiers>> {
    ...
    const tiers = await fetchTiers(db, ModifierTierModel, id);
  ...
  async update(
    ...
    const nonColumnKeys = MODIFIER_CONFIG.nonColumnKeys ?? [];
    ...
    try {
      await withTransaction(db, async (txDb) => {
        ...
        await insertTiers(txDb, ModifierTierModel, id, tiers);
      });
    } catch (error) {
      const message = error instanceof Error ? error.message : "Database error";
      return { success: false, stage: "database", error: message };
  ...
  async checkFieldAvailable(
    ...
    const result = await selectManyWorkflow(db, ModifierModel, conditions);
...
const _tierOrchestration = createTierOrchestration<HasTiers<ModifierWithTiers>>(
  ModifierTierModel,
