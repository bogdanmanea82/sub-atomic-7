...
import { link, deleteForm, escapeHtml, ICON_EDIT, ICON_COPY } from "../../sub-atoms";
...
import { statusFormSection, statusBadge } from "./status-section";
import { MODIFIER_TABS } from "./modifier-tabs";
...
/** Derives current 3-state status from form field values and archivedReason. */
function deriveFormState(view: ModifierFormView): "active" | "disabled" | "archived" {
  if (view.archivedReason) return "archived";
  const isActive = view.fields.some((f) => f.name === "is_active" && (f.value === true || f.value === "true"));
...
export function createPage(
  ...
  const statusHtml = statusFormSection(deriveFormState(view), view.statusReason);
...
export function editPage(
  ...
  const currentState = deriveFormState(view);
  const reasonValue = currentState === "archived" ? (view.archivedReason ?? "") : (view.statusReason ?? "");
  const statusHtml = statusFormSection(currentState, reasonValue);
  const tierHtml = tierFormSection(view.tierRows, view.tierFieldMeta);
